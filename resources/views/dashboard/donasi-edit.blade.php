...
@section('body')
    <form action="/metode-donasi/{{ $donasi->id }}/edit" method="POST" id="form" enctype="multipart/form-data"
        ...
        <div class="flex space-x-3">
            {{-- metode --}}
            <input type="text" id="metode" name="metode" placeholder="Metode donasi"
                value="{{ old('metode', $donasi->metode) }}" oninput="buatSlug(this, 'slug')"
        ...
        {{-- rekening --}}
        <input type="text" name="rekening" id="rekening" value="{{ old('rekening', $donasi->rekening) }}"
        ...
        {{-- link --}}
        <input type="text" name="link" id="link" value="{{ old('link', $donasi->url) }}" placeholder="Tautan"
