...
<div class="space-y-7">
    ...
    <div class="space-y-2">
        <button wire:ignore wire:click='changeMode'
            class="px-4 py-2 hover:bg-amber-300 hover:text-neutral-800 rounded-full flex gap-1 items-center">
        ...
        <div class="flex md:flex-row flex-col gap-1 w-full">
            <div class="relative w-full">
                ...
                <button wire:ignore wire:click='clearText'
                    class="absolute top-3 right-3 flex items-center gap-1 p-2 rounded-full hover:bg-amber-400 hover:text-neutral-800 group">
                    <div class="group-hover:block hidden text-sm">Hapus</div>
                    <i data-lucide='x' class="size-4 my-0.5"></i>
                </button>
                {{-- textarea --}}
                <label for="baseText"
                    class="absolute top-3 left-3 bg-amber-400 text-neutral-800 px-3 py-0.5 rounded-full first-letter:font-serif first-letter:font-bold first-letter:text-xl">
                ...
                <div class="{{ $target == 'latin' ? 'javanese text-lg' : '' }}">
                    <textarea wire:ignore.self wire:model.live.debounce.500ms='baseText' name="baseText" id="baseText"
                        class="min-h-32 p-4 pt-14 border border-neutral-200 dark:border-zinc-700 dark:bg-zinc-900 rounded-2xl w-full resize-none focus:outline-amber-400"
            ...
            <div class="relative w-full">
                ...
                <label for="result"
                    class="absolute top-3 left-3 bg-white dark:bg-zinc-900 px-3 py-0.5 rounded-full first-letter:font-serif first-letter:font-bold first-letter:text-xl">
                ...
                <div class="{{ $target == 'aksara-jawa' ? 'javanese text-lg' : '' }}">
                    <textarea wire:ignore.self wire:model.live.debounce.500ms='result' name="result" id="result"
                        class="min-h-32 p-4 pt-14 bg-amber-100 dark:bg-zinc-800 rounded-xl w-full resize-none focus:outline-none" readonly
