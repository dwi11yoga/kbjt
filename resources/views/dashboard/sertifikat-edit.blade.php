...
@section('body')
    ...
    @enderror">
                ...
                <option {{ old('role', $sertifikat->role) == 'kontributor' ? 'selected' : '' }} value="kontributor">
                    Kontributor</option>
                <option {{ old('role', $sertifikat->role) == 'pengurus' ? 'selected' : '' }} value="pengurus">
                    Pengurus
                </option>
                <option {{ old('role', $sertifikat->role) == 'semua' ? 'selected' : '' }} value="semua">
                    Kontributor dan
                    pengurus</option>
            </select>
        ...
        @enderror">
                <option value="">Pilih</option>
                <option {{ old('rule', $sertifikat->rule) == 'keanggotaan' ? 'selected' : '' }} value="keanggotaan">
                    Lama terdaftar sebagai anggota
                </option>
                <option {{ old('rule', $sertifikat->rule) == 'kontribusi' ? 'selected' : '' }} value="kontribusi">
                    Total kontribusi anggota
                </option>
                <option class="pengurus" {{ old('rule', $sertifikat->rule) == 'kontribusiPengurus' ? 'selected' : '' }}
        ...
        <div class="bg-white px-5 py-4 rounded-xl">
            ...
            <input type="number" min="1" id="requirement" name="requirement" placeholder="Masukkan angka"
                value="{{ old('requirement', $sertifikat->requirement) }}" oninput="buatSlug(this, 'slug')"
        ...
        <div class="bg-white px-5 py-4 rounded-xl">
            ...
            <input type="number" min="0" id="reward" name="reward" placeholder="Masukkan angka"
                value="{{ old('reward', $sertifikat->reward) }}" oninput="buatSlug(this, 'slug')"
