...
@section('body')
    ...
    @if ($post->dokumentasi == true)
        <div class="w-full border border-gray-200 rounded-2xl py-6 px-7 space-y-3">
            ...
            <a href="/cari?keyword=dokumentasi%3A&filter=artikel">
                <button
                    class="py-3 px-3 mt-3 rounded-lg bg-amber-400 hover:outline hover:outline-2 hover:outline-offset-2 outline-amber-400">Cek
                    sekarang!</button>
            </a>
        </div>
    @endif
    ...
    {{-- artikel lain --}}
    @if (!empty($artikelLain) && isset($post->status))
        <div class="">
