...
@section('body')
    <form action="" method="POST" class="space-y-3">
                ...
                <div class="md:col-span-4 col-span-5 bg-white px-5 py-4 space-y-2 rounded-xl">
                    ...
                    <div class="">
                        ...
                        <input type="text" id="nama" name="nama" placeholder="Masukkan nama"
                            value="{{ old('nama') }}" oninput="buatSlug(this, 'slug')"
                    ...
                    <div class="">
                        ...
                        <input type="text" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi singkat"
                            value="{{ old('deskripsi') }}" oninput="buatSlug(this, 'slug')"
        ...
        @enderror">
                <option value="">Pilih</option>
                <option {{ old('rule') == 'keanggotaan' ? 'selected' : '' }} value="keanggotaan">Lama
                    terdaftar
                    sebagai anggota
                </option>
                <option {{ old('rule') == 'definisi' ? 'selected' : '' }} value="definisi">Jumlah
                    definisi yang user
                    buat
                </option>
                <option {{ old('rule') == 'kosakata' ? 'selected' : '' }} value="kosakata">Jumlah
                    kosakata yang user
                    tambahkan</option>
                <option {{ old('rule') == 'editKosakata' ? 'selected' : '' }} value="editKosakata">
                    Jumlah kosakata
                    yang user
                    edit</option>
                <option {{ old('rule') == 'laporan' ? 'selected' : '' }} value="laporan">Jumlah laporan
                    yang user
                    kirim
                </option>
                <option {{ old('rule') == 'artikel' ? 'selected' : '' }} value="artikel">Jumlah artikel
                    yang user
                    publikasikan</option>
                <option {{ old('rule') == 'totalViewKosakata' ? 'selected' : '' }} value="totalViewKosakata">Total
                    jumlah view pada
                    semua kosakata user</option>
                <option {{ old('rule') == 'viewKosakata' ? 'selected' : '' }} value="viewKosakata">
                    Jumlah view pada
                    satu
                    kosakata</option>
                <option {{ old('rule') == 'totalViewBlog' ? 'selected' : '' }} value="totalViewBlog">
                    Total jumlah
                    view pada
                    semua artikel user</option>
                <option {{ old('rule') == 'viewBlog' ? 'selected' : '' }} value="viewBlog">Jumlah view
                    pada satu
                    artikel
                </option>
            </select>
