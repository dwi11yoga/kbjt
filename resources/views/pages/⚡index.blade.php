            ...
            <div class="grid grid-cols-6 gap-4">
                ...
                <div class="md:col-start-2 md:col-span-3 col-span-6 space-y-2">
                    ...
                    <div class="md:hidden flex justify-center">
                        <a href="#"
                            class="block w-fit bg-amber-400 p-4 rounded-full hover:outline hover:outline-2 hover:outline-offset-2 hover:outline-amber-400 active:bg-amber-500">Temukan
                ...
                <div class="md:block hidden col-span-1">
                    <div class="bg-amber-400 rounded-2xl p-6 sticky top-24 space-y-2">
        ...
        <div class="md:px-24">
            ...
            $faq = [
                [
                    ...
                    'content' => 'Pengguna dapat mendaftar terlebih dahulu, kemudian mulai menambahkan kata atau definisi baru. Kontribusi yang telah dikirimkan akan ditampilkan di halaman kosakata dan akan mendapatkan status "terverifikasi" setelah melalui proses validasi oleh pengurus atau moderator.',
                ],
                [
                    'id' => 2,
                    'title' => 'Apakah terdapat keuntungan bagi kontributor?',
                    'content' => 'Ya. Setiap kontribusi yang diberikan akan menghasilkan poin yang menentukan level kontributor. Pada pencapaian level tertentu, kontributor akan menerima sertifikat penghargaan. Selain itu, tersedia sistem pencapaian (achievement) khusus yang dapat diraih melalui penyelesaian aktivitas tertentu. Pengguna lain juga dapat memberikan donasi langsung sebagai bentuk apresiasi atas kontribusi yang telah diberikan.',
                ],
                [
                    'id' => 3,
                    'title' => 'Apakah platform ini tersedia secara gratis?',
                    'content' => 'Ya, platform ini dapat digunakan secara gratis oleh seluruh pengguna. Meskipun demikian, pengguna dapat turut mendukung keberlangsungan platform melalui donasi. Dana yang terkumpul akan digunakan untuk membiayai kebutuhan hosting dan domain.',
                ],
                [
                    'id' => 4,
                    'title' => 'Apa yang harus dilakukan jika menemukan kata atau definisi yang tidak sesuai?',
                    'content' => 'Pengguna dapat melaporkan kata atau definisi yang tidak sesuai melalui fitur laporkan kesalahan. Laporan tersebut akan ditindaklanjuti oleh pengurus atau moderator, dan perbaikan akan dilakukan apabila diperlukan.',
                ],
                [
                    'id' => 5,
                    'title' => 'Apakah pengguna harus memahami Bahasa Jawa untuk menggunakan platform ini?',
                    'content' => 'Tidak. Platform ini dirancang untuk ramah pengguna, termasuk bagi mereka yang baru memulai mempelajari Bahasa Jawa. Tersedia fitur terjemahan dan contoh penggunaan yang dapat membantu pengguna dalam memahami setiap kosakata.',
                ],
            ];
            ?>
            ...
            @foreach ($faq as $d)
                <div>
                    <button onclick="toggleClass('accordion-content-{{ $d['id'] }}', 'hidden')"
                        id="accordion-header-{{ $d['id'] }}"
                        class="text-neutral-600 flex justify-between w-full bg-white p-5 border border-neutral-200 hover:bg-amber-100">
                        <div id="accordion-title-{{ $d['id'] }}" class="">
                            <i data-lucide='help-circle' class="inline-block"></i>
                            <i data-feather='help-circle' class="inline-block mr-3"></i>
                            {{ $d['title'] }}
                        </div>
                        <div id="accordion-show-{{ $d['id'] }}">
                            <i data-lucide='chevron-down'></i>
                            <i data-feather='chevron-down' class="absolute top-5 right-6"></i>
                        </div>
                    </button>
                    <div id="accordion-content-{{ $d['id'] }}"
                        class="hidden py-5 pl-6 pr-4 border border-neutral-200 text-neutral-500">
                        {!! $d['content'] !!}
                    </div>
                </div>
            @endforeach
